...
/// - Prefabを VRCInstantiate で生成し、指定親(Zone)の子として配置する。
/// - 生成先はローカルのみ（同期は ChemElementSpawner の同期変数に任せる）
/// - Prefab未設定/未一致の場合は null を返し、Spawner 側の既存モデル移動ロジックにフォールバックさせる。
/// </summary>
public class ChemRuntimeToolSpawner : UdonSharpBehaviour
{
    ...
    {
        if (!enablePrefabSpawn) return null;
